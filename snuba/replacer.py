...
class ReplacerWorker(AbstractBatchWorker[Replacement]):
    def __init__(self, clickhouse, dataset, metrics=None) -> None:
    ...
    def process_message(self, message) -> Optional[Replacement]:
    ...
    def flush_batch(self, batch: Sequence[Replacement]) -> None:
        for replacement in batch:
            ...
            t = time.time()
            logger.debug("Executing replace query: %s" % (replacement.insert_query_template % query_args))
            self.clickhouse.execute_robust(replacement.insert_query_template % query_args)
